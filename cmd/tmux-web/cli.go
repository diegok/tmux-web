...
// `tmux-web enroll --name phone | qrencode -t ansiutf8` works without any
// --quiet flag.
...
Usage:
  tmux-web serve   --host <hostname> [--dev | --self-signed | --tls-cert F --tls-key F]
  tmux-web enroll  --name <device>
  tmux-web devices
  tmux-web revoke  <device-id>
...
Run "tmux-web <command> -h" for a command's flags.
`
...
func run(args []string, stdout, stderr io.Writer) int {
	...
	case "help", "-h", "-help", "--help":
		...
		return 0
	default:
		fmt.Fprintf(stderr, "tmux-web: unknown command %q\n\n", cmd)
		printUsage(stderr)
		return 2
	}
}
...
func cmdEnroll(args []string, stdout, stderr io.Writer) int {
	fset := newFlagSet("enroll", stderr, "tmux-web enroll --name <device>")
...
func cmdDevices(args []string, stdout, stderr io.Writer) int {
	fset := newFlagSet("devices", stderr, "tmux-web devices")
	socket := socketFlag(fset)
	operands, code, ok := parseFlags(fset, args)
	if !ok {
		return code
	...
	if len(operands) > 0 {
		return usageError(stderr, fset, "devices takes no positional arguments, got %q (to remove one: tmux-web revoke %s)", operands[0], operands[0])
	}
	...
	if len(resp.Devices) == 0 {
		...
		fmt.Fprintln(stderr, "no devices enrolled; run: tmux-web enroll --name <device>")
		return 0
	}
...
func cmdRevoke(args []string, stdout, stderr io.Writer) int {
	fset := newFlagSet("revoke", stderr, "tmux-web revoke <device-id>")
	socket := socketFlag(fset)
	operands, code, ok := parseFlags(fset, args)
	if !ok {
		return code
	...
	case 1:
	case 0:
		return usageError(stderr, fset, "revoke needs a device id; run: tmux-web devices")
	default:
...
//
// "dial unix /run/user/1000/tmux-web.sock: connect: no such file or directory"
...
func (c *adminClient) describeFailure(err error) error {
	...
	case errors.Is(err, fs.ErrNotExist):
		return fmt.Errorf("tmux-web does not appear to be running: there is no admin socket at %s.\n"+
			"  start it with:  tmux-web serve --host <hostname>\n"+
			"  already running elsewhere? point at its socket with --socket <path>", c.socket)
	case errors.Is(err, syscall.ECONNREFUSED):
		return fmt.Errorf("tmux-web is not running: %s is left over from a daemon that exited.\n"+
			"  start it with:  tmux-web serve --host <hostname>", c.socket)
...
func socketFlag(fset *flag.FlagSet) *string {
	return fset.String("socket", "", "admin socket path (default: $XDG_RUNTIME_DIR/tmux-web.sock)")
}
