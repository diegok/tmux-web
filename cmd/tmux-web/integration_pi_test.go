...
func TestPiExtensionWiresTheFiveEventsToTheReportArgv(t *testing.T) {
	...
	// stops existing.
	if first.stdin != `{"tmux_web_is_idle":true}` {
		t.Errorf("session_start stdin = %q, want %q -- events.go discriminates pi's session_start on exactly this key, and reads anything else as `working`",
			first.stdin, `{"tmux_web_is_idle":true}`)
	}
...
func requirePi(t *testing.T) string {
	...
	if err != nil {
		t.Skipf("SKIPPING %s: pi is not installed (%v). The pi extension's wiring -- that it registers five events and spawns `tmux-web report --agent pi --event <name>` -- is NOT covered on this machine. Its filters still are, in internal/integrations/pi.test.ts",
			t.Name(), err)
	}
	return path
}
...
func piEnv(t *testing.T, dir, log string) []string {
	...
	return []string{
		...
		"PI_OFFLINE=1",
		"OPENAI_API_KEY=tmux-web-test-invalid",
