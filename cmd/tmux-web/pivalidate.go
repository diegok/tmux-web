...
const piProbeProvider = "tmux-web-install-probe"
...
func runPiProbe(artifact []byte) (piVerdict, string) {
	...
	dir, err := os.MkdirTemp("", "tmux-web-pi-probe-")
	if err != nil {
	...
	ext := filepath.Join(agent, "extensions", "tmux-web.ts")
	...
	cmd := exec.CommandContext(ctx, bin,
		...
		"--no-session", "-p", "tmux-web install probe")
	cmd.Dir = work
...
func validatePiArtifact(dir string, artifact []byte) error {
	...
	return fmt.Errorf("this install cannot be checked, so it has not been made: %s.\n"+
		...
		"    tmux-web install-integration --agent pi <dir>\n"+
