...
// 1, everywhere --
//
//	Error: Failed to load extension "…/tmux-web.ts": Extension does not export a
...
func TestPiProbeVerdictReadsARealPiRun(t *testing.T) {
	const ext = "/tmp/tmux-web-pi-probe-123/agent/extensions/tmux-web.ts"
	...
	if !strings.Contains(piProbeProvider, "tmux-web") {
...
func TestPiGlobalInstallRefusesWhatPiRefuses(t *testing.T) {
	home := globalHome(t)
	stubPiProbe(t, piRefuses, `Failed to load extension "…/tmux-web.ts": Extension does not export a valid factory function`)
...
func TestPiValidationRunsOnlyWhereItIsNeeded(t *testing.T) {
	...
	t.Run("a global remove does not validate", func(t *testing.T) {
		...
		if _, err := os.Stat(filepath.Join(home, "pi-agent", "extensions", "tmux-web.ts")); err == nil {
...
func TestThePiProbeCarriesNothingOfTheUsersOwn(t *testing.T) {
	...
	t.Setenv("OPENAI_API_KEY", "tmux-web-must-not-leak-this")
...
//
// The schema number is written down in four places -- Go's tmuxWebSchema, the
// managed header of pi.ts and of opencode.js, and queue.ts's TMUX_WEB_SCHEMA,
...
func TestTheIntegrationSourcesCarryTheSchemaTheInstallerWrites(t *testing.T) {
	want := "tmux-web-schema: " + strconv.Itoa(tmuxWebSchema)
	...
	wantConst := "export const TMUX_WEB_SCHEMA = " + strconv.Itoa(tmuxWebSchema)
