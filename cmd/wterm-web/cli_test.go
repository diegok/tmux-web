...
func newAdminDir(t *testing.T) *admin {
	...
	return &admin{store: st, enroller: auth.NewEnroller(st), socket: filepath.Join(dir, "wterm-web.sock"), dir: dir}
}
...
func TestCommandsFailLoudlyWhenTheDaemonIsNotRunning(t *testing.T) {
	...
	t.Cleanup(func() { os.RemoveAll(dir) })
	missing := filepath.Join(dir, "wterm-web.sock")
	...
	for _, args := range [][]string{
		...
		r.wantStderrContains(t, missing, "not appear to be running", "wterm-web serve", "--socket")
	}
}
...
func TestAStaleSocketIsReportedAsADeadDaemon(t *testing.T) {
	...
	r := runCLI("devices", "--socket", a.socket)
	r.wantCode(t, 1)
	r.wantStderrContains(t, a.socket, "not running", "wterm-web serve")
}
