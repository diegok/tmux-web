...
// `wterm-web install-integration` writes the three agent integrations into a
...
// wtermSchema is the version in the managed header. Bump it when the shape of
...
// separate.
var managedHeader = regexp.MustCompile(`managed by tmux-web \(wterm-schema: (\d+)\)`)
...
// binary moved.
const claudeScriptName = "wterm-report.sh"
...
//     factory: "Failed to load extension ".../wterm-queue.ts": Extension does
...
const inlineBanner = `// -- queue.ts, inlined by ` + "`wterm-web install-integration`" + ` -----------------
//
...
//
// Our own path first, because the wterm-web being run is the wterm-web the user
...
func resolveBinary() string {
	...
	if p, err := exec.LookPath("wterm-web"); err == nil {
		return p
	}
	return "wterm-web"
}
...
func mergeClaudeHooks(original []byte, script string, remove bool) ([]byte, bool, error) {
	...
	// older wterm-web may have left an entry somewhere this one no longer
