...
func requireOpencode(t *testing.T) string {
	...
	if err != nil {
		t.Skipf("SKIPPING %s: opencode is not installed (%v). The plugin's wiring -- that it loads, registers three hooks and spawns `wterm-web report --agent opencode --event <name>` for six events and no others -- is NOT covered on this machine. Its two gates still are, in internal/integrations/opencode.test.ts",
			t.Name(), err)
	}
	return path
}
...
func installPluginShim(t *testing.T, dir string) {
	...
	if err := os.WriteFile(filepath.Join(pluginDir, "wterm.js"),
