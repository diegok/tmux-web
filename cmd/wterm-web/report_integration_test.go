...
//     #{@wterm_agent} up the hierarchy -- pane, then window, then session, then
...
//     `set -t work @wterm_agent SESSIONLEVEL`,
//     `list-panes -a -F '#{@wterm_agent}'` printed SESSIONLEVEL for both panes.
...
func TestReportReachesTheSnapshot(t *testing.T) {
	...
	if reports[otherPane] != "" {
		t.Fatalf("the pane nobody reported on reads %q: the write was not scoped to this pane (`set` without `-p` sets a SESSION option, which tmux resolves through #{@wterm_agent} on every pane of the session; `set -p` without `-t` lands on the active pane)",
			reports[otherPane])
	}
	if len(rows) != 2 {
...
func TestAStateOnlyReportIsThreeFields(t *testing.T) {
	...
	if !regexp.MustCompile(`^1;idle;[0-9]{13}$`).MatchString(stored) {
		t.Fatalf("@wterm_agent = %q, want 1;idle;<13 digits> and no trailing separator", stored)
	}
...
func TestAnUnknownStateWritesNothing(t *testing.T) {
	...
	if reports[paneID] != "" {
		t.Fatalf("@wterm_agent = %q after an unknown state; want nothing written at all", reports[paneID])
	}
